...
if __name__ == "__main__":
    ...
    assert is_user_in_group("sub_child_user", parent) == True, "Should find user in nested subgroup"

    # Test Case 2: User is in immediate subgroup
    assert is_user_in_group("sub_child_user", child) == True, "Should find user in immediate subgroup"

    # Test Case 3: User is in current group
    assert is_user_in_group("sub_child_user", sub_child) == True, "Should find user in current group"

    # Test Case 4: User not in any group
    assert is_user_in_group("nonexistent_user", parent) == False, "Should return False for nonexistent user"

    # Test Case 5: None user
    assert is_user_in_group(None, parent) == False, "Should return False for None user"
